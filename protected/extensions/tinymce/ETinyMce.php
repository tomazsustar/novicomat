...
class ETinyMce extends CInputWidget
{
   ...
   *
    ...
    * @var array
    */
   private $switchLabels = array(0=>'HTML', 1=>'HTML mode');
   ...
   private $labelReadmore= '"Preberi več"';
...
EOP;
      ...
      if ($this->useSwitch && !$this->readOnly) {
		...
		$label = $this->switchLabels[($this->mode=='html'?0:1)];
         ...
         $uri=";"; 
         $toggleEditor = CHtml::link($label, 'javascript:'.$uri, $switchOptions);
                  
         //preberi več
         $switchOptions = array_merge(array('id'=>$id."_readmore",
             'onclick'=>"insertReadmore('$id')"), $css);
         $readmore = CHtml::link($this->labelReadmore, 'javascript:'.$uri, $switchOptions);
                          
         $buttons = CHtml::tag('div', array(), CHtml::tag('span', array(), $toggleEditor.$readmore));
         
         $html = CHtml::tag('div', array(), $textarea.$buttons);
         /*
         $label = $this->switchLabels[($this->mode=='html'?0:1)];
         $css = ($this->labelClass !== '') ? array('class'=>$this->labelClass) : array('style'=>$this->labelStyle);
	...
	private function js(){ return "
			...
			function insertReadmore(editor) {
			    editorInstance=tinyMCE.get(editor);
			    var content = editorInstance.getContent();
				// if (content.match(/<hr\s+id=(\"|')system-readmore(\"|')\s*\/*>/i)) {
				if (editorInstance.dom.get('system-readmore')) {
					//alert('črta');
					//setCursor(editorInstance, editorInstance.dom.get('system-readmore'), true);
					editorInstance.dom.remove('system-readmore');
				} else {
